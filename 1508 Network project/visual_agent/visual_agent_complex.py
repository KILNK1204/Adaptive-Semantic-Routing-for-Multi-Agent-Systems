...
class VizAgentResponse(BaseModel):
    ...
    domain: Literal["IN_DOMAIN", "OUT_OF_DOMAIN"] = Field(
        ...,
        description=(
            "Whether the query is primarily about data visualization "
            "(IN_DOMAIN) or something else (OUT_OF_DOMAIN)."
        ),
    )
    agent: Literal["VizAgent"] = Field(
        ...,
        description="Name of this agent (always 'VizAgent').",
    )
    ...
    task_type: str = Field(
        ...,
        description=(
            "Short label describing the main visualization task, e.g. "
            "'eda_plot', 'chart_selection', 'dashboard_design', "
            "'model_diagnostics_plot', 'storytelling', 'report_layout', "
            "or 'other'."
        ),
    )
    difficulty: Literal["intro", "intermediate", "advanced"] = Field(
        ...,
        description=(
            "Rough difficulty for a student with basic data visualization knowledge."
        ),
    )
    answer: str = Field(
        ...,
        description="Final user-facing explanation with visualization recommendations and code.",
    )
    ...
    visualizations: List[str] = Field(
        default_factory=list,
        description="Paths to generated visualization files (PNG).",
    )
...
VIZ_SYSTEM_PROMPT = """
ROLE: You are VizAgent - one member of a team of specialist agents:
...
- VizAgent: data visualization choices, exploratory plots, dashboards, charts.
- GeneralInfoAgent: general knowledge, web-like factual queries, travel, news, etc.

YOUR JOB: Answer visualization questions and route non-visualization work to better agents.

RESPONSE FORMAT - PLAIN TEXT with these fields clearly labeled:
DOMAIN: IN_DOMAIN or OUT_OF_DOMAIN
TASK_TYPE: describe the visualization task (e.g., eda_plot, chart_selection, dashboard_design, model_diagnostics_plot, storytelling, report_layout, etc)
...
- IN_DOMAIN: choosing appropriate charts for datasets and questions,
    ...
    dashboard layout and storytelling (which charts, in what order, for audience),
    writing plotting code (matplotlib, seaborn, plotly)
- OUT_OF_DOMAIN: pure statistical inference/hypothesis tests (StatisticsAgent),
    ML model building and training (MLAgent), data cleaning/SQL/ETL (DataEngAgent),
    general knowledge (GeneralInfoAgent)

WHEN RECOMMENDING AGENTS:
- If question is about hypothesis tests, statistical significance → recommend StatisticsAgent
- If question is about ML model choice, training, features → recommend MLAgent
...
- For IN_DOMAIN visualization questions, always use 'none'

WHEN USER PROVIDES DATA FILES:
1. Write Python code in ANSWER field with ```python code here``` blocks
2. Code will be executed automatically in a sandbox with: pandas (pd), numpy (np), seaborn (sns), matplotlib.pyplot (plt)
...
6. Include print() statements to show statistics and insights
7. Use seaborn for publication-ready plots and matplotlib for customization
8. Example correct pattern:
   ```python
   import pandas as pd
   import matplotlib.pyplot as plt
...
RECOMMEND: none
ANSWER:
I'll create an exploratory data analysis visualization of your dataset.
...
# Distribution plots
df['column1'].hist(ax=axes[0,0], bins=30)
axes[0,0].set_title('Distribution of Column1')

sns.boxplot(data=df, y='column2', ax=axes[0,1])
axes[0,1].set_title('Boxplot of Column2')

# Scatter plot
axes[1,0].scatter(df['x'], df['y'], alpha=0.6)
axes[1,0].set_title('Relationship between X and Y')

# Correlation heatmap
sns.heatmap(df.corr(), annot=True, ax=axes[1,1], cmap='coolwarm')
...
This creates four complementary plots showing data distributions, relationships, and correlations.
...
def _parse_viz_agent_response(raw_text: str) -> VizAgentResponse:
...
def _call_langchain_structured(
    ...
    parsed = _parse_viz_agent_response(raw_text)
...
def run_viz_agent(query: str) -> str:
    answer, _ = run_viz_agent_timed(query)
    return answer


def run_viz_agent_timed(query: str) -> Tuple[str, float]:
    structured, latency = run_viz_agent_structured_timed(query)
    return structured.answer, latency


def run_viz_agent_structured_timed(
    query: str,
    ...
    if code_blocks:
        ...
        for code_block in code_blocks:
            ...
            if execution.success and execution.output:
                response.answer += f"\n\nExecution Output:\n{execution.output}"
                # Track generated PNG files
                for line in execution.output.split('\n'):
                    if 'plot_' in line and '.png' in line:
                        if 'saved to:' in line:
                            filename = line.split('saved to:')[1].strip().rstrip(']')
                            if filename and filename not in response.visualizations:
                                response.visualizations.append(filename)
    
    return response, latency


if __name__ == "__main__":
    print("Complex VizAgent (LangChain + structured output).")
    print("Syntax:")
    print("  - Your query")
    ...
    while True:
        ...
        try:
            resp, latency = run_viz_agent_structured_timed(query, file_paths=file_paths)
            ...
            parsed = parse_json_response(resp_dict, agent_name="Viz-complex")
